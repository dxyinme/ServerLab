/*
 * Copyright (c) 2017, 哈尔滨天疆科信信息技术有限公司.
 * 版权所有. All rights reserved.
 *
 * 注意：
 * 本代码仅限于哈尔滨天疆科信信息技术有限公司内部开发使用，
 * 严禁外泄或将其用于任何其他目的。
 */
package com.spboot.demo.LogicAPI;
 ...
 * 这里的的开始时间戳，一般是我们的id生成器开始使用的时间，由我们程序来指定的（如下下面程序IdWorker类的startTime属性）。 41位的时间戳，可以使用69年，年T = (1L <<
 * 41) / (1000L * 60 * 60 * 24 * 365) = 69<br> 10位的数据机器位，可以部署在1024个节点，包括5位datacenterId和5位workerId<br>
 * 12位序列，毫秒内的计数，12位的计数顺序号支持每个节点每毫秒(同一机器，同一时间截)产生4096个ID序号<br> 加起来刚好64位，为一个Long型。<br>
 * SnowFlake的优点是，整体上按照时间自增排序，并且整个分布式系统内不会产生ID碰撞(由数据中心ID和机器ID作区分)，并且效率较高，
 * 经测试，SnowFlake每秒能够产生26万左右的ID。
 */
public class SnowflakeIdWorker {
     ...
     * 开始时间戳 (2017-03-14)
     */
    private final static long twepoch = 1489420800L;
    /**
     * 机器id所占的位数
     */
